    ...
    reader.onload = (ev) => {
      try {
        ...
        const blocks = text
          ...
          .map((b) => b.trim())
          .filter((b) => b);
        const importedQuestions = blocks
          .map((block) => {
            const lines = block
              .split("\n")
              .map((l) => l.trim())
  ...
  const handlePublish = async (status) => {
    ...
    try {
      ...
      if (initialData?.id) {
        ...
        if (!testMeta.start_password)
          throw new Error("Start Password required for new exams.");
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-900">
              {testMeta.title || "Untitled Assessment"}
            </h1>
            <p className="text-sm font-semibold text-slate-500">
              {initialData?.id ? "Editing Draft" : "Exam Builder Mode"}
            </p>
          </div>
        </div>
        <div className="flex bg-slate-100 p-1 rounded-lg">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-bold transition-all ${activeTab === "setup" ? "bg-white text-cyan-600 shadow-sm" : "text-slate-500"}`}
          >
            <Settings size={16} /> Setup
          </button>
          <button
            onClick={() => setActiveTab("mcq")}
            className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-bold transition-all ${activeTab === "mcq" ? "bg-white text-cyan-600 shadow-sm" : "text-slate-500"}`}
          >
            <FileText size={16} /> MCQs{" "}
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-bold transition-all ${activeTab === "coding" ? "bg-white text-cyan-600 shadow-sm" : "text-slate-500"}`}
          >
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm max-w-2xl mx-auto space-y-5">
            <div>
              ...
              <input
                ...
                className="w-full border border-slate-200 rounded-lg px-4 py-3 focus:border-cyan-500 outline-none bg-slate-50 font-semibold"
              />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setTestMeta({ ...testMeta, starts_at: e.target.value })
                  }
                  className="w-full border border-slate-200 rounded-lg px-4 py-3 focus:border-cyan-500 outline-none bg-slate-50 font-semibold"
                />
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  value={
                    initialData?.id
                      ? testMeta.start_password.startsWith("$2b$")
                        ? ""
                        : testMeta.start_password
                      : testMeta.start_password
                  }
                  onChange={(e) =>
                    setTestMeta({ ...testMeta, start_password: e.target.value })
                  }
                  className="w-full border border-slate-200 rounded-lg px-4 py-3 focus:border-cyan-500 outline-none bg-slate-50 font-semibold text-cyan-700"
                  placeholder={
                    initialData?.id ? "Leave blank to keep existing" : ""
                  }
                />
              </div>
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setTestMeta({ ...testMeta, end_password: e.target.value })
                }
                className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2.5 text-sm font-bold focus:outline-none focus:border-cyan-500"
              />
            </div>
          </div>
        )}
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
              <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                <FileText size={20} className="text-cyan-600" /> Multiple Choice
                Questions
              </h2>
              <div className="flex gap-2">
                <label className="flex items-center gap-1.5 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200 px-4 py-2 rounded-lg text-sm font-bold transition-all cursor-pointer shadow-sm">
                  ...
                  <input
                    ...
                    accept=".txt"
                    className="hidden"
                    onChange={handleAikenImport}
                  />
                </label>
                <button
                  onClick={addQuestion}
                  className="flex items-center gap-1.5 bg-cyan-600 hover:bg-cyan-700 text-white px-4 py-2 rounded-lg text-sm font-bold transition-all shadow-sm"
                >
            ...
            {questions.map((q, idx) => (
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-4">
                  <span className="bg-slate-100 text-slate-500 font-black px-3 py-1 rounded-md text-xs">
                    Q{idx + 1}
                  </span>
                  <input
                    type="text"
                    value={q.section}
                    onChange={(e) =>
                      updateQuestion(q.id, "section", e.target.value)
                    }
                    className="bg-slate-50 border rounded px-2 py-1 text-xs font-bold text-slate-600 outline-none"
                ...
                <textarea
                  ...
                  rows="3"
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:border-cyan-500 outline-none resize-none mb-3 font-semibold"
                />
                  ...
                  {["A", "B", "C", "D"].map((opt) => (
                    <div key={opt} className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        value={q[`opt${opt}`]}
                        onChange={(e) =>
                          updateQuestion(q.id, `opt${opt}`, e.target.value)
                        }
                        className="w-full bg-slate-50 border rounded p-1.5 text-xs focus:border-cyan-500 outline-none"
                      />
                    </div>
                  ))}
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <select
                    value={q.ans}
                    onChange={(e) =>
                      updateQuestion(q.id, "ans", e.target.value)
                    }
                    className="bg-emerald-50 text-emerald-700 border border-emerald-200 font-bold text-xs rounded p-1 outline-none"
                  >
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-end gap-3">
          <button
            ...
            className="flex items-center gap-2 px-6 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-lg transition-colors"
          >
            <Save size={18} />{" "}
          ...
          <button
            ...
            disabled={isSubmitting}
            className="flex items-center gap-2 px-8 py-2.5 bg-cyan-600 hover:bg-cyan-700 text-white font-bold rounded-lg shadow-md transition-all"
          >
