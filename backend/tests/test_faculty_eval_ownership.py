...
def _session(db, exam_id, sid="sess_owner_1", student_id="23-TEST-01", submitted=True, payload=None, subjective=None, created_at=None):
    mdb = get_mongo_db()
    doc = {
        "_id": sid,
        "id": sid,
        ...
        "submission_payload": json.dumps(payload) if payload is not None else (json.dumps({"mcqs": {}, "coding": {}}) if submitted else None),
...
def _staff(db, sid, module="MAS701"):
    mdb = get_mongo_db()
    pw_hash = bcrypt.hashpw(b"test1234", bcrypt.gensalt(rounds=12)).decode("utf-8")
    doc = {
        "_id": sid,
        "id": sid,
        "name": sid,
...
class TestFacultyOwnershipIsolation:
    def test_faculty_creates_reads_updates_own_coding(self, client, db, faculty_bearer_headers):
        eid = _exam(db)
        mdb = get_mongo_db()
        mdb["coding_problems"].insert_one({
            "_id": "cp_a1", "exam_id": eid, "title": "P", "description": "d",
        })
        sid = _session(db, eid)

        save1 = client.post(
            f"/admin/exams/{eid}/evaluate/{sid}",
        ...
        assert _eval_count(db, sid, "staff_faculty_test") == 1

        list_resp = client.get(f"/admin/exams/{eid}/evaluate", headers=faculty_bearer_headers)
    ...
    def test_two_faculty_coding_coexist(self, client, db):
        ...
        mdb = get_mongo_db()
        mdb["coding_problems"].insert_one({
            "_id": "cp_a2", "exam_id": eid, "title": "P", "description": "d",
        })
        sid = _session(db, eid, sid="sess_owner2")
        ...
        assert _eval_count(db, sid, f1["id"]) == 1
        assert _eval_count(db, sid, f2["id"]) == 1

        # Faculty 1 sees only their own marks
        mine = client.get(f"/admin/exams/{eid}/evaluate", headers=f1["headers"]).json()
        assert mine["data"][0]["current_coding_marks"] == {"cp_a": 30.0}

        # Faculty 2 sees only their own marks
        theirs = client.get(f"/admin/exams/{eid}/evaluate", headers=f2["headers"]).json()
    ...
    def test_faculty_cannot_read_other_facultys_evaluation(self, client, db):
        ...
        mdb = get_mongo_db()
        mdb["coding_problems"].insert_one({
            "_id": "cp_b", "exam_id": eid, "title": "P", "description": "d",
        })
        sid = _session(db, eid, sid="sess_owner3")
        ...
        mine = client.get(f"/admin/exams/{eid}/evaluate", headers=f2["headers"]).json()
        assert mine["data"][0]["current_coding_marks"] == {}

    def test_faculty_spoof_faculty_id_is_ignored(self, client, db):
        eid = _exam(db)
        sid = _session(db, eid, sid="sess_spoof")
        ...
        spoof = client.get(
            f"/admin/exams/{eid}/evaluate?faculty_id={f2['id']}",
            headers=f1["headers"],
        ).json()
    ...
    def test_cross_module_faculty_blocked(self, client, db, other_faculty_bearer_headers):
        eid = _exam(db)  # MAS701
        sid = _session(db, eid, sid="sess_xm")
        resp = client.post(
            f"/admin/exams/{eid}/evaluate/{sid}",
...
class TestSubjectiveOwnership:
    def test_subjective_evaluations_coexist_and_isolated(self, client, db):
        eid = _exam(db, eid="E_subj")
        mdb = get_mongo_db()
        mdb["subjective_questions"].insert_one({
            "_id": "sq_1", "exam_id": eid, "section": "Theory", "text": "Q",
        })
        sid = _session(db, eid, sid="sess_subj")
        ...
        r1 = client.get(f"/admin/exams/{eid}/evaluate", headers=f1["headers"]).json()
        r2 = client.get(f"/admin/exams/{eid}/evaluate", headers=f2["headers"]).json()
...
class TestPendingAndPartial:
    def test_no_evaluation_is_pending(self, client, db, admin_bearer_headers):
        eid = _exam(db, eid="E_pend")
        _session(db, eid, sid="sess_pend")
        resp = client.get(f"/admin/exams/{eid}/analytics", headers=admin_bearer_headers)
    ...
    def test_partial_coding_only(self, client, db, admin_bearer_headers, faculty_bearer_headers):
        eid = _exam(db, "E_partial")
        mdb = get_mongo_db()
        mdb["coding_problems"].insert_one({
            "_id": "cp_p", "id": "cp_p", "exam_id": eid, "title": "P", "description": "d",
        })
        sid = _session(db, eid, sid="sess_part")
        client.post(
            f"/admin/exams/{eid}/evaluate/{sid}",
...
class TestAdminVisibilityAndSwitching:
    def test_admin_can_switch_faculty_context(self, client, db, admin_bearer_headers, faculty_bearer_headers):
        eid = _exam(db, "E_admswitch")
        mdb = get_mongo_db()
        mdb["coding_problems"].insert_one({
            "_id": "cp_sw", "exam_id": eid, "title": "P", "description": "d",
        })
        sid = _session(db, eid, sid="sess_sw")
        ...
        mdb["faculty_evaluations"].update_one(
            {"session_id": sid, "faculty_id": "staff_faculty_test"},
        ...
        mdb["faculty_evaluations"].update_one(
            {"session_id": sid, "faculty_id": f_other["id"]},
            {"$set": {"created_at": 200.0}},
        )

        # no param -> default (earliest = staff_faculty_test => 1)
        data = client.get(f"/admin/exams/{eid}/analytics", headers=admin_bearer_headers).json()["data"]
        ...
        data2 = client.get(
            f"/admin/exams/{eid}/analytics?faculty_id={f_other['id']}",
    ...
    def test_admin_selecting_foreign_faculty_rejected(self, client, db, admin_bearer_headers):
        ...
        eid = _exam(db, "E_foreign")
        _session(db, eid, sid="sess_foreign")
        foreign = _staff(db, "sf_foreign", module="MAS702")
        resp = client.get(
            f"/admin/exams/{eid}/analytics?faculty_id={foreign['id']}",
            headers=admin_bearer_headers,
        )
        assert resp.status_code == 403

    def test_admin_default_legacy_when_no_faculty(self, client, db, admin_bearer_headers):
        eid = _exam(db, "E_legdefault")
        _session(db, eid, sid="sess_ld")
        data = client.get(f"/admin/exams/{eid}/analytics", headers=admin_bearer_headers).json()["data"]
        assert data["faculty_context"]["default_faculty_id"] is None
        assert data["faculty_context"]["is_legacy"] is True
...
class TestLegacyContext:
    def test_legacy_only_for_admin_when_no_faculty_rows(self, client, db, admin_bearer_headers, faculty_bearer_headers):
        eid = _exam(db, "E_leg")
        sid = _session(db, eid, sid="sess_leg")
        mdb = get_mongo_db()
        mdb["exam_sessions"].update_one(
            {"_id": sid},
            {"$set": {
                "coding_evaluation": json.dumps({"legacy_cp": 7}),
                "subjective_evaluation": json.dumps({"legacy_sq": 3}),
            }},
        )

        # Admin sees legacy values while no faculty eval exists
        admin = client.get(f"/admin/exams/{eid}/analytics", headers=admin_bearer_headers).json()["data"]
        ...
        fac = client.get(f"/admin/exams/{eid}/evaluate", headers=faculty_bearer_headers).json()
...
class TestIntegrity:
    def test_duplicate_saves_never_duplicate_rows(self, client, db, faculty_bearer_headers):
        eid = _exam(db, "E_dup")
        sid = _session(db, eid, sid="sess_dup")
        for _ in range(5):
            client.post(
                f"/admin/exams/{eid}/evaluate/{sid}",
    ...
    def test_module_reassignment_preserves_history(self, client, db, admin_bearer_headers, faculty_bearer_headers):
        eid = _exam(db, "E_reas")
        sid = _session(db, eid, sid="sess_reas")
        f = _staff(db, "reas_fac")
        client.post(
            f"/admin/exams/{eid}/evaluate/{sid}",
            json={"coding_marks": {"c": 5}}, headers=f["headers"],
        )
        assert _eval_count(db, sid, f["id"]) == 1
        ...
        blocked = client.get(f"/admin/exams/{eid}/evaluate", headers=f["headers"])
